...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 这里的beanPostProcessor都是系统默认的，不是用户自定义的。比如负责注入ApplicationContext引用到各
			// 种Aware中的ApplicationContextAwareProcessor容器后处理器。
			...
			try {
				...
				// 注册bean后处理器，将实现了BeanPostProcessor接口的bean找到。先将实现了PriorityOrdered接口的
				// bean排序并注册到容器BeanFactory中，然后将实现了Ordered接口的排序并注册到容器中，最后注册剩下的。
				...
				// 注册事件监听器，也就是所有实现了ApplicationListener的类。会将监听器加入到事件广播器ApplicationEventMulticaster
				// 中，所以在广播时就可以发送消息给所有监听器了。
				registerListeners();
...
protected void prepareRefresh() {
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		// 刷新beanFactory
		// AbstractAutowireCapableBeanFactory中实现，并创建beanFactory
		refreshBeanFactory();
		//获取beanFactory并进行返回
